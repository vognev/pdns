...
RUN apt-get update -qq && apt-get install -y curl gnupg apt-transport-https\
 ...
 && apt-get update -qq && apt-get install -y curl php8.0-{cli,mbstring,intl}\
...
RUN curl -s https://packages.sury.org/php/apt.gpg | apt-key add -\
 && apt-get update -qq && apt-get install -o Dpkg::Options::="--force-confold" -y php8.0-{xdebug,pcov}\
 && phpdismod -v 8.0 -s cli xdebug\
